...
if len(sys.argv) != 5:
	...
	print(" ----------------------------------------------------------\n")
	print(" Usage:\n\tpython filterACS.py PATH_SIM run_start run_stop npos\n example:\n\tpython filterACS.py ../Simulations/SIM_DIR/10000/122keV/ 1 1 9\n")
	print(" ----------------------------------------------------------\n")
	print(" Parameters:\n\t- PATH_SIM = path where the BoGEMMS simulation runs are stored\n")
	print(" \t- run_start = initial run number\n")
	print(" \t- run_start = final run number\n")
	print(" \t- npos = number of positions\n")
	print("\n")
	exit()
...
input_params = []
file_input = PATH_SIM + "/pos1/run" + str(run_start) + "/runSiPM.txt"
...
path_sim = PATH_SIM
N_in_fits = int(N_in/num_tasks) # number of events in a single file FITS
N_tot = N_in_fits*num_tasks*N_runs
N_tot = N_in*N_runs

biases = ["left", "right", "forward", "back"]
if bias in biases: bias_label = "_" + bias
else: bias_label = ""
refl2_dir = "/REFL2_TYPE" + str(refl2_type)

path_output = "."+"/SIM"+str(sim_id)+"/GEOM_TYPE"+str(geom_type)+"/BGO_ABSL_TYPE"+str(bgo_absl_type)+"/REFL_TYPE"+str(refl_type)+refl2_dir+"/"+str(N_in)+"/"+source+"/collimated"+bias_label+"/"+str(run_start)+"-"+str(run_stop)+"/"
path_runs = "."+"/SIM"+str(sim_id)+"/GEOM_TYPE"+str(geom_type)+"/BGO_ABSL_TYPE"+str(bgo_absl_type)+"/REFL_TYPE"+str(refl_type)+refl2_dir+"/"+str(N_in)+"/"+source+"/collimated"+bias_label+"/"
...
#for jpos in range(4, 5):
	pos = jpos + 1
	pos_dir = "/pos"+str(pos)
	
	for jrun in range(run_start, run_stop + 1):
		rundir = "/run"+str(jrun)
		N_tasks = len(glob.glob1(path_sim+pos_dir+rundir,"*0.task*.fits*"))
		N_in_per_task = int(N_in / N_tasks) # number of events per task

		for task in range(N_tasks):
			N_fits = len(glob.glob1(path_sim+pos_dir+rundir,"*.task"+str(task)+".fits.gz"))

			for jfits in range(N_fits):

				print('%%%%%%%%%%%%%% READING THELSim FILE: ', path_sim+pos_dir+rundir+'/xyz.'+str(jfits)+'.task'+str(task)+'.fits.gz')
				hdulist = fits.open(path_sim+pos_dir+rundir+'/xyz.'+str(jfits)+'.task'+str(task)+'.fits.gz')
					...
					while evt_id[temp_index] == evt_id[temp_index+1]:
						...
						if (where_scint[0].size):
							...
							if (where_SiPM[0].size):
								...
								vecSiPMNabsErr.append(np.sqrt(nabs_SiPM))
								# Apply PDE
								summ = 0
...
vecNPos = np.array(vecNPos)
vecScintEdep = np.array(vecScintEdep)
vecSiPMEdep = np.array(vecSiPMEdep)
vecSiPMNabs = np.array(vecSiPMNabs)
vecSiPMNabsErr = np.array(vecSiPMNabsErr)
vecSiPMNabsPDE = np.array(vecSiPMNabsPDE)
vecScintNopt = np.array(vecScintNopt)
vecScintNoptErr = np.array(vecScintNoptErr)
vecNPosSingle = np.array(vecNPosSingle)
vecSiPMEdepSingle = np.array(vecSiPMEdepSingle)
vecScintEventIDSingle = np.array(vecScintEventIDSingle)
...
f_out.write("# EventID Pos Edep_scint[keV] Nabs_SiPM_PDE Nabs_SiPM N_opt Edep_SiPM[keV]\n")
for je in range(len(vecScintEventID)):

	f_out.write('{0:30}'.format(str(vecScintEventID[je])))
	f_out.write('{0:30}'.format(str(vecNPos[je])))
