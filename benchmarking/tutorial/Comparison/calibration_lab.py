...
for dataset in datasets:
    ...
    for source in sources:
        ...
        for line in lines:
            ...
            # Cut spectrums around the photopeak
            ...
            # Perform the fit of the photopeak with the selected model
    ...
    # Perform the fit of the energy - channel relation
    priors = [(1, 0.3), (-10, 2)]
    initial_guess = [mean for (mean, std) in priors]
    ...
    for f in os.listdir(path):
        ...
        if os.path.isfile(file):
            ...
            energy = m * channel + q
            # Propagate the errors of the fit parameters
            energy_err = np.sqrt((em*channel)**2 + (eq)**2)
            # Evaluate the energy range due to the systematic uncertainty
            energy_up, energy_down = energy + energy_err, energy - energy_err
            # Interpolate the counts at energy_up and energy_down to get the systematic uncertainty on the counts
            counts_sys = np.interp(energy, energy_up, counts) - np.interp(energy, energy_down, counts)
            # Propagate the errors on the counts (systematic and statistical)
    ...
    for source in sources:
        ...
        # Rescale counts due to different acquisition times
...
### Fit the energy resolution against the energy ###
