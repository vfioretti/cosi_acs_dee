...
# Path where the simulation is run
...
for jrun in range(run_start, run_stop + 1):
	...
	if gorun == 1:
		# Select the configuration file and the macro file
		file_conf = "SiPM.conf"
		file_mac = ""
		...
		if isslurm == 0: subprocess.Popen(["thelsim", file_conf, "0", file_mac])	
