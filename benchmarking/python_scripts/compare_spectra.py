...
# Read simulated energy resolution and fit parameters from file
...
# Read experimental energy resolution and fit parameters from file
energy, resolution, resolution_err, dataset_name = [], [], [], []
...
for dataset in datasets:
    ...
    for source in sources:
        # Read files (source and background, lab and sim)
        ...
        # Rebin lab data according to the simulated energy bins (to have the same energy bins for sim and lab)
        energy = energy_sim
        # Rebin the counts three times: once for the energy, once for the top error, and once for the bottom error
        ...
        # Rebin bkg data according to the simulated energy bins
        counts_bkgs, counts_bkgs_top, counts_bkgs_bot = [], [], []
        # I need to do it for each background spectrum separately
        ...
        # Systematic errors (due to time variations) --> the difference between the maximum and minimum counts in the background spectra
        counts_sys_temp = np.max(counts_bkgs, axis=0)-np.min(counts_bkgs, axis=0)
        # Ststematic errors (due to energy calibration) --> the difference between the maximum and minimum counts in the background spectra for the top and bottom errors
        counts_sys_cal = np.abs(counts_bkg_top - counts_bkg_bot)/2
        # Statistical + Systematic (time variations) + systematic (energy calibration)
        counts_bkg_err = np.sqrt((counts_bkg_stat)**2 + (counts_sys_temp)**2 + (counts_sys_cal)**2)
        ...
        # Get the energy resolution for the source (if multiple lines, take the first one)
        lines = source['lines']
        first_line = lines[0]
        ...
        # Select the counts in the FWHM range
